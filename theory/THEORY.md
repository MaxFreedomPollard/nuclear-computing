...
in the thin limit. The scale of the problem follows immediately. For ¹⁷⁸ᵐ²Hf with its measured β = 12.39 (see `/gates`), a cross section of one barn would demand N_A > 8.1×10²² excited nuclei per cm², about two centimeters of *pure, fully inverted* solid hafnium isomer; no macroscopic quantity of ¹⁷⁸ᵐ²Hf has ever existed. A millibarn cross section demands a thousand times more. The chart's own best case is now known ([/photon](../photon/)): of every level restoring coincidence in ENSDF whose gateway width is measured, the one asking least of the absorber still needs 1×10²⁴ inverted nuclei per cm², 23 cm of pure isomer, so this wall is not a bound on a hypothetical state but a measurement on the real list. Only two escapes exist: a resonantly enormous σ_trig (the ²²⁹Th optical transition reaches effective resonant cross sections many orders above geometric, which is why lasers can drive single nuclei), or a channel whose cross section is already hundreds of barns (Section 4).
...
- **Terminal.** ¹⁸⁰ᵐTa releases 75.3 keV against a trigger costing at least 1.01 MeV. No arrangement of terminal gates can cascade; they are readouts, not logic. Photon number gain without level restoration is bookkeeping, not amplification: the energy ledger must also close, β Ē_out > C_in E_trig, with Ē_out usable at the next stage.
...
**The convertible class, counted.** The search this paragraph asked for has been run ([/photon](../photon/), numbers in [`photon/results.md`](../photon/results.md)). Over the adopted level schemes of the 415 isomers that hold a bit for at least a second, with every gateway above each isomer in an observed class and a spin parity allowed class, every release cascade followed through the adopted gammas, both recoils and both thermal Doppler widths applied, and a 1 km/s rotor allowed to close the remainder as Moon's did in 1951, there are 12470 coincidences of a release line with a gateway absorption compatible with resonance within the quoted uncertainties, 12402 of them heterogeneous and 3775 closed loops, of which the data are precise enough to call 0 resonant, since ENSDF quotes most energies to tens or hundreds of electronvolts and a rotor closes a few. The class exists, as candidates. Every member of it then meets Section 1.1: among those whose gateway width is measured, the least demanding asks for 1×10²⁴ inverted nuclei per cm², a solid 23 cm thick made entirely of the isomer, because a Doppler broadened MeV resonance is barns wide. The convertible class is therefore a table of energies and not of gates, and the photon amplifier, if it exists, is not a coincidence waiting in the chart but a state whose trigger cross section is large for a reason, which is the NEEC class of 34 gateways within 30 keV of their isomer that the same census lists.

**Inhibition, which the neutron sector lacked.** The neutron calculation of Section 4 found no signal controlled inversion in that sector. Isomers have one by construction: an isomer with two releasing gateways whose cascades differ can be emptied down a path that does not emit the signal line, so a control on the second gateway is a veto. The census counts 214 such isomers. Inversion therefore comes off the list of properties the keystone must be discovered to have; it is a property of any isomer with two gateways, and the search that must still succeed is the one for gain.
