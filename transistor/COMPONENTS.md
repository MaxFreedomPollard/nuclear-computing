...
| clock | crystal oscillator | divide by N on the Poisson stream: jitter exactly 1/√N | demonstrated (fig. 13A; 0.1 percent clock = 10⁶ decays per tick) |
...
| rewritable working memory | DRAM + sense amps | a self exciting loop through a saturable stage: latches iff loop gain Γ > 1 | keystone gated (fig. 13B: the fork at Γ = 1, measured) |
