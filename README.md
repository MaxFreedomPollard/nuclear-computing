...
A gate that meets only the leak condition is a **memory or neuron**; a gate that meets both is a **logic amplifier**, the thing a scalable computer is built from.
...
| **⁹³ᵐMo** (6.85 h) | plausible | few | NEEC trigger **unconfirmed** |
| **¹⁷⁸ᵐ²Hf** (2.45 MeV, 31 yr) | leak trivially beaten | $\sim 12$ (big gain) | trigger **contested, unproven** |
...
**Phase B. Settle the keystone.** Measure $(\sigma_{\text{trig}}, \beta, C_{\text{in}})$ for the best NEEC or IGE candidate (precision EBIT or Penning trap NEEC; resonant IGE of a mid energy isomer) and locate it on the keystone figure.
- *Kill criterion B (the big one):* if **no** isomer can be shown to satisfy both the leak condition and $\Gamma>1$ at any achievable flux, then nuclear computing is permanently confined to the **stochastic tier plus memory**, a real but bounded result. The theory's *high* ambition lives or dies here.
...
**Repository deliverables that feed this roadmap:** `/theory` (full derivations), `/transport` (MCNP and Geant4 models of $\mathcal{G}$ and the soft threshold neuron), `/gates` (the keystone criterion evaluated against ENSDF and ENDF data for every candidate isomer), `/references`.
...
|---|---|---|---|---|---|
| ²²⁹ᵐTh | 8.355733(8) eV | about 29 min (vac.) | $4.0\times10^{-4}$ | memory or qubit, soft threshold | **proven (laser)**, $\beta\approx1$ |
| ⁹³ᵐMo | 2.4 MeV | 6.85 h | $2.8\times10^{-5}$ | candidate logic | NEEC **contested** |
| ¹⁷⁸ᵐ²Hf | 2.446 MeV | 31 yr | $7\times10^{-10}$ | high gain logic | IGE **contested** |
| ¹⁸⁰ᵐTa | 75 keV | $>10^{16}$ yr | about 0 | stored energy | IGE candidate |
| ⁵⁷Fe | 14.4 keV | 98 ns | | Mössbauer, Tier 2 coherence | **routine** |
