...
A unit is self sustaining over a mission time $\tau_m$ if every subsystem's demand is met by the decay inventory alone at all times:

$$ a_i\, \lambda_0\, 2^{-t/T} \;\geq\; D_i \quad \text{for all } i \text{ and all } t \leq \tau_m, $$
...
2. **Compute shell.** The gate medium: scattering and absorbing microstructure whose geometry *is* the weight matrix $\mathcal{G}$, with coincidence volumes and saturable channels arranged per the transport theory. The problem is ground into the shell at manufacture, the way an ASIC's function is etched: **a problem in a can.**
3. **Trim layer.** Resonant channels (Mössbauer foils) whose detunings are set through the sealed wall: Zeeman coils outside the vessel shift lines by about 0.7 natural linewidths per tesla, and piezoelectric Doppler rings modulate them at kHz to MHz (THEORY.md 8.2). The ampoule is fixed function in topology, field programmable in weights.
...
- A 1 GBq alpha core depositing 5.5 MeV per decay into krypton at 30 percent VUV conversion emits about $2\times10^{14}$ photons per second near 148 nm: a bright lamp by any ordinary standard.
- Spread over the excimer bandwidth (about 10 nm, i.e. $1.4\times10^{14}$ Hz), that is a spectral flux near 1 photon cm⁻² s⁻¹ Hz⁻¹.
- The nuclear line's integrated cross section is $(\lambda^2/2\pi)\,\Gamma_{\text{rad}} \approx 2\times10^{-15}$ cm² Hz. The excitation rate per nucleus is their product: about $3\times10^{-15}$ s⁻¹, **one write quantum per voxel of $10^8$ nuclei every five weeks.**

The mismatch is the ratio of the lamp's bandwidth to the nuclear linewidth, about eighteen orders of magnitude, and no achievable activity closes it. The conclusion is structural, not incremental: **broadband self pumping of neV lines is dead**, and any self written nuclear memory waits on a *narrowline* internal source (a nuclear pumped laser, demonstrated technology in rare gases since the 1970s but never at VUV linewidths of interest) or on the keystone. Publishing this closes the most obvious "why not just..." a referee could raise, and it dictates the design that follows.
...
The boundary translates by the theory of THEORY.md Section 8. For a detected boundary budget of $10^7$ events per second (a 1 GBq class core with percent level boundary collection):
